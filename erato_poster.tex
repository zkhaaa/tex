        ...
        \begin{mybox}{Introduction}
            \begin{itemize}
                ...
                \begin{itemize}
                    ...
                    \item[$\bullet$] Produces uncertainty with posterior distribution
                    \item[$\bullet$] Can always physical ($|\theta| \leq 1$)
                \end{itemize}
            \end{itemize}
        ...
        \begin{mybox}{Model and problem setting}
            \begin{itemize}
                \item \textbf{Parametric model}
                    ...
                    \begin{equation}
                        \rho_\theta=\frac{1}{2}\left(I+\theta_x\sigma_x+\theta_y\sigma_y+\theta_z\sigma_z\right),\quad \sum_{i=1}^{3}\theta_i^2\leq1.
                    \end{equation}
                ...
                \begin{equation}
                    k_i \thicksim \mathrm{Binomial}(n_i,p_i), p_i=\frac{1}{2}(1+\theta_i) \quad \text{for } i=x,y,z.
                \end{equation}
                \item \textbf{Problem}
                \item [] Bayesian methods vs. Frequentist methods.
        \begin{columns}
            \begin{column}{0.51\linewidth}
                \begin{itemize}
                    \item [] Expected a posteriori estimate (EAP)
            ...
            \begin{column}{0.49\linewidth}
                \begin{itemize}
                    ...
                    \item[] Liner inversion (LI)
                    \item[] Maximum likelihood estimate (MLE)
                \end{itemize}
            \end{column}
        \end{columns}
        \ \\
            \item \textbf{Bayesian Framework and prior model}
            \item[] Assume parameter $\theta$ obeys a distribution:
            \begin{equation}
                    (\theta_x,\theta_y,\theta_z) = r \cdot \overrightarrow{u} 
            \end{equation}
            \item[$\bullet$] $r\thicksim \mathrm{Beta}(\alpha,\beta)$ represent purity.
            \item[$\bullet$] $\overrightarrow{u}\thicksim \mathrm{vMF}(\kappa,\mu)$ represent direction.
        \end{itemize}
        \end{mybox}
            
            \begin{mybox}{Simulation example}
                \begin{itemize}
                    \item \textbf{Getting measurement outcomes}
                    \item[] Two states $|\theta|=0.99$ (pure), $|\theta|=0.6$ (mixed).
                    \item[] n measurements per X/Y/Z basis with $n=10,50$.
...
\end{center}   
                ...
                \begin{columns}
                    ...
                    \begin{column}{0.45\linewidth}
                        \begin{itemize}
                            ...
                            \item LI: non-physical ($|\theta| > 1$)   
                        \end{itemize}
                    \end{column}
                \end{columns}
                \begin{shortbox}
                    \begin{center}
                        Bayesian method (EAP) can outperform maximum likelihood estimation under limited sample size.
                    \end{center}
                \end{shortbox}
            \end{mybox}
    ...
    \begin{column}{0.5\linewidth}
        \begin{mybox}{Performance under less prior knowledge}
            \begin{itemize}
                \item \textbf{Non-informative prior on $r$, weakly informative prior on $\overrightarrow{u}$}
                \item[] Setting: $(\alpha,\beta)=(0.5,0.5)$, $\kappa=0.5$ and $\mu=(1,0,0)$, i.e.,  little prior knowledge exists.
                \begin{center}
                    \includegraphics[width=0.8\linewidth]{theta_mse_i2.png}
                \end{center}
                \begin{columns}
                    \begin{column}{0.45\linewidth}
                        \begin{itemize}
        ...
        \begin{mybox}{Performance under prior misspecification}

                \begin{itemize}
                \item \textbf{Prior misspecification}
                \item Setting: $(\alpha,\beta)=(1,4)$, $\kappa=4$ and $\mu=(\red{-1},0,0)$, i.e., opposite direction.
                \begin{center}
                    \includegraphics[width=0.8\linewidth]{theta_mse_i_oppsite.png}
                \end{center}
                \item[] Under severely misspecified prior, MAP degrades significantly, while EAP is less sensitive.
                \item\text{ Check posterior:} A weak \red{secondary mode} provide a signal of prior-data mismatch.
                \begin{center}
        ...
        \begin{mybox}{Conclusion and outlook}
            \begin{itemize}
                \item Bayesian estimation yields stable and physical estimates under limited data.
                \item EAP is insensitive to prior choice and achieves low variance.
                \item Posterior provides uncertainty quantification and detects prior-data mismatch.
                \item[]\textbf{Outlook}
